...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut active: HashSet<(isize, isize, isize, isize)> = HashSet::new();
    let mut inactive: HashSet<(isize, isize, isize, isize)> = HashSet::new();

    for (j, line) in buf.lines().enumerate() {
        for (i, ch) in line.chars().enumerate() {
            match ch {
                '#' => active.insert((i as isize, j as isize, 0, 0)),
                '.' => inactive.insert((i as isize, j as isize, 0, 0)),
                _ => unreachable!(),
            };
        }
    }
